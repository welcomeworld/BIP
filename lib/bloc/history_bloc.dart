...
class HistoryBloc extends Bloc {
  ...
  final List<MediaPageHistory> _cacheList = [];
  BehaviorSubject<List<MediaPageHistory>> listSubject = BehaviorSubject();
  ...
  Future<void> requestHistory() async {
    final result = await _database.queryMediaPageHistory(
        key: _searchKey, pageNumber: _pageNumber++);
    if (result.isEmpty) {
      listEnd.add(true);
    } else {
      if (_pageNumber == 2) {
